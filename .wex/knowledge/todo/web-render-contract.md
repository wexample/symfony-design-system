...
2. **Split packages** – create two tiny packages (`php-web-render-node`, `js-web-render-node`) that expose the same schema/DTOs/types. They only host the contract: no rendering logic.
...
4. **Protocol repo** – extract the language-agnostic schema/JSON spec into `packages/COMMON/repo/web-render-node`, so PHP/JS packages just implement that reference.
