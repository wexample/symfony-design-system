...
export default class extends PageManagerComponent {
  ...
  protected async activateListeners(): Promise<void> {
    ...
    const options = this.renderData?.requestOptions as PanelRequestOptionsInterface | undefined;
  ...
  private onClickContent = async (event: Event) => {
    ...
    const closeLink = target.closest('.panel-close a') as HTMLElement | null;
    if (!closeLink) {
      return;
    }
